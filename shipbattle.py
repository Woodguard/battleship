...
def ai_pass():
    ...
    while ai_guessing:
        ...
        if player.board[ai_guess_y][ai_guess_x] == s_ship:
            ...
            if destroy:
                print("\nОператор уничтожил ваше судно (X: %s, Y: %s)." % (ai_guess_x, ai_guess_y))
            else:
                print("\nОператор нанёс повреждения вашему судну (X: %s, Y: %s)." % (ai_guess_x, ai_guess_y))
            break
            ...
            print("\nОператор промазал (X: %s, Y: %s)." % (ai_guess_x, ai_guess_y))
            break
...
while game:
    ...
    while guessing:
        ...
        if not (guess_x in range(size)) or not (guess_y in range(size)):
            print("\nЭта клетка слишком далеко, чтобы поразить ее! Введите другие координаты")
            continue
            ...
            print("\nВам не удалось вычислить судно противника", end=" ")
            press_ent()

        else:
            print("\nВы уже поразили этот сектор, пожалуйста, укажите другие координаты")
            continue
        break

    if len(ai.spawned) == 0:
        input("Вам удалось уничтожить все корабли Оператора! Ничего, в следующий раз он отомстит. Нажмите ENTER чтобы завершить игру.")
        break

    if len(player.spawned) == 0:
        print("Оператор одержал верх. Он с удовольствием примет еще один вызов")
